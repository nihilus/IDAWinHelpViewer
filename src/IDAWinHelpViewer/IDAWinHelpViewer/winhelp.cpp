 ...
 *				2010 Fabio Fornaro IDA Pro 'IDA WinHelp Viewer' Plugin SDK compliance adaptations <fabio.fornaro@gmail.com>
 *
...
extern HWND hIDAWindow;
extern WINHELP_WINDOW win;
extern WINHELP_WNDPAGE wpage;
extern HWND hHiddenKeywordsList;
...
HLPFILE_LINK* WINHELP_FindLink(WINHELP_WINDOW* win, LPARAM pos)
{
	HLPFILE_LINK*           link;
	...
	cp = SendMessageW(hTextWnd , EM_CHARFROMPOS,
		0, (LPARAM)&mouse_ptl);
	...
	for (link = win->page->first_link; link; link = link->next)
	{
		if (link->cpMin <= cp && cp <= link->cpMax)
		{
			...
			SendMessageW(hTextWnd , EM_POSFROMCHAR,
				(LPARAM)&char_ptl, cp);
			SendMessageW(hTextWnd , EM_POSFROMCHAR,
...
*           WINHELP_InitFonts
*/
void WINHELP_InitFonts(HWND hWnd)
{
	WINHELP_WINDOW *_win = &win;//(WINHELP_WINDOW*) GetWindowLongPtr(hWnd, 0);
	LOGFONT logfontlist[] = {
		{-10, 0, 0, 0, 400, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 32, "Courier"},
		{-12, 0, 0, 0, 700, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 32, "Courier"},
		{-12, 0, 0, 0, 700, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 32, "Courier"},
		{-12, 0, 0, 0, 400, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 32, "Courier"},
		{-12, 0, 0, 0, 700, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 32, "Courier"},
		{-10, 0, 0, 0, 700, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 32, "Courier"},
		{ -8, 0, 0, 0, 400, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 32, "Courier"}};
#define FONTS_LEN (sizeof(logfontlist)/sizeof(*logfontlist))

		static HFONT fonts[FONTS_LEN];
		static BOOL init = 0;

		_win->fonts_len = FONTS_LEN;
		_win->fonts = fonts;
										  ...
										  LONG cb, LONG* pcb)
{
	...
	buff = (BYTE*) strrpl((char*)buff,0xBE,0x2D);
...
void WINHELP_SetupText(HWND hTextWnd, WINHELP_WINDOW* win, ULONG relative)
{
...
static INT WINHELP_MessageBoxIDS_s(UINT ids_text, LPCSTR str, UINT ids_title, WORD type)
{
	...
	qsnprintf(newtext, sizeof(text), text, str);
...
HLPFILE* WINHELP_LookupHelpFile(LPCSTR lpszFile)
{
	...
	*/
	...
	{
		qstpncpy(szAddPath, Globals.active_win->page->file->lpszPath,sizeof(szAddPath));
		p = strrchr(szAddPath, '\\');
		if (p) *p = 0;
	}
...
HLPFILE_WINDOWINFO*     WINHELP_GetWindowInfo(HLPFILE* hlpfile, LPCSTR name)
{
	...
	if (!mwi.name[0])
	{
		qstrncpy(mwi.type, "primary",sizeof(mwi.type));
		qstrncpy(mwi.name, "main",sizeof(mwi.name));
		if (hlpfile && hlpfile->lpszTitle[0])
		{
			char        tmp[128];
			LoadString(Globals.hInstance, STID_WINE_HELP, tmp, sizeof(tmp));
			qsnprintf(mwi.caption, sizeof(mwi.caption), "%s %s - %s", hlpfile->lpszTitle, tmp, hlpfile->lpszPath);
		}
		else
...
BOOL WINHELP_CreateHelpWindow(WINHELP_WNDPAGE* wpage, int nCmdShow, BOOL remember)
{
	...
	if (!win)
	{
		...
		win = (WINHELP_WINDOW*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(WINHELP_WINDOW));
		if (!win) return FALSE;
	...
	*/
	...
	if (bPopup)
	{
		...
		win->hShadowWnd = CreateWindowEx(WS_EX_TOOLWINDOW, (LPCSTR)SHADOW_WIN_CLASS_NAME,
			"", WS_POPUP | WS_VISIBLE,
							...
							int nCmdShow)
{
	...
	return InvalidateRect(hTextWnd, NULL, TRUE);
}
...
BOOL WINHELP_HandleTextMouse(WINHELP_WINDOW* win, UINT msg, LPARAM lParam)
{
	HLPFILE*                hlpfile;
	HLPFILE_LINK*           link;
	BOOL                    ret = FALSE;
	...
	case WM_LBUTTONDOWN:
		if ((link = WINHELP_FindLink(win, lParam)))
		{
			...
			switch (link->cookie)
			{
			case tagHlpFileLink::hlp_link_link:
...
static LRESULT CALLBACK WINHELP_ButtonBoxWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_WINDOWPOSCHANGING:
		...
		for (button = win->first_button; button; button = button->next)
		{
			...
			if (!button->hWnd)
			{
				button->hWnd = CreateWindow((LPCSTR)STRING_BUTTON,button->lpszName,
	...
	case WM_PAINT:
		...
		for (i = 0; i < Globals.history.index; i++)
		{
			...
			else
			{
				...
				qstrncpy(&buffer[len], Globals.history.set[i].page->lpszTitle, sizeof(buffer) - len);
...
*
*/
void cb_KWBTree(void *p, void **next, void *cookie)
{
...
BOOL WINHELP_CreateIndexWindow(BOOL is_search)
{
	...
	//	qstrncat(buf, Globals.active_win->info->caption,sizeof(buf));
...
static BOOL WINHELP_RegisterWinClasses(void)
{
	...
	class_shadow.lpszClassName     = (LPCSTR)SHADOW_WIN_CLASS_NAME;
...
int PASCAL WinHelpWinMain(HINSTANCE hInstance, HINSTANCE prev, LPSTR cmdline, int show)
{
	MSG                 msg;
	LONG                lHash = 0;
	HLPFILE*            hlpfile;
	...
	if (*cmdline)
	{
		...
		hlpfile=WINHELP_LookupHelpFile("x86eas.hlp");
		if (!hlpfile) return 0;
	}
	else hlpfile =  HLPFILE_ReadHlpFile("x86eas.hlp");//NULL;
	...
	{
		if (dll->tclass & DC_INITTERM) dll->handler(DW_TERM, 0, 0);
	}
	return 0;
}
