 ...
 *				2010 Fabio Fornaro IDA Pro 'IDA WinHelp Viewer' Plugin SDK compliance adaptations
 *
                           ...
                           int leaf, void** next)
{
    ...
    LONG lTest = (INT)GET_UINT((const BYTE*)p, 0);
...
HLPFILE_PAGE *HLPFILE_PageByHash(HLPFILE* hlpfile, LONG lHash, ULONG* relative)
{
    ...
    ptr = (BYTE*)HLPFILE_BPTreeSearch(hlpfile->Context, LongToPtr(lHash), comp_PageByHash);
    if (!ptr)
    {
...
static int comp_FindSubFile(void *p, const void *key,
                            int leaf, void** next)
{
    *next = (char *)p+strlen((const char*)p)+(leaf?5:3);
    WINE_TRACE("Comparing '%s' with '%s'\n", (char *)p, (char *)key);
    return strcmp((const char*)p, (const char*)key);
}
...
static BOOL HLPFILE_FindSubFile(HLPFILE* hlpfile, LPCSTR name, BYTE **subbuf, BYTE **subend)
{
    ...
    ptr = (BYTE*)HLPFILE_BPTreeSearch(hlpfile->file_buffer + GET_UINT(hlpfile->file_buffer, 4),
                                              ...
                                              BYTE** alloc)
{
    ...
    case 0: /* uncompressed */
        ...
        *alloc = NULL;
        break;
    case 1: /* RunLen */
        dst = *alloc = (BYTE*)HeapAlloc(GetProcessHeap(), 0, sz);
        if (!dst) return NULL;
        HLPFILE_UncompressRLE(src, src + csz, *alloc, sz);
        break;
    case 2: /* LZ77 */
        sz77 = HLPFILE_UncompressedLZ77_Size(src, src + csz);
        dst = *alloc = (BYTE*)HeapAlloc(GetProcessHeap(), 0, sz77);
        if (!dst) return NULL;
    ...
    case 3: /* LZ77 then RLE */
        sz77 = HLPFILE_UncompressedLZ77_Size(src, src + csz);
        tmp = (BYTE*)HeapAlloc(GetProcessHeap(), 0, sz77);
        if (!tmp) return FALSE;
...
static BOOL HLPFILE_RtfAddRawString(struct RtfData* rd, const char* str, size_t sz)
{
    ...
    {
        char*   snew = (char*)HeapReAlloc(GetProcessHeap(), 0, rd->data, rd->allocated *= 2);
        if (!snew) return FALSE;
        rd->ptr = snew + (rd->ptr - rd->data);
        rd->data = snew;
    }
...
static BOOL HLPFILE_RtfAddText(struct RtfData* rd, const char* str)
{
    ...
    for (last = p = str; *p; p++)
    {
        ...
        {
            static char         xx[8];
            rlen = qsnprintf(xx, sizeof(xx), "\\'%x", *(const BYTE*)p);
            replace = xx;
        }
        else switch (*p)
        {
    ...
    for (i = 0; i < hs_num; i++)
    {
        ...
        case 0xC8:
            hslink = (HLPFILE_HOTSPOTLINK*)
                HLPFILE_AllocLink(rd, tagHlpFileLink::hlp_link_macro, str, -1, 0, 0, 1, -1);
            break;

        case 0xE6:
        case 0xE7:
            hslink = (HLPFILE_HOTSPOTLINK*)
				HLPFILE_AllocLink(rd, (start[7 + 15 * i + 0] & 1) ? tagHlpFileLink::hlp_link_link : tagHlpFileLink::hlp_link_popup,
                                            ...
                                            const void* pict, unsigned nc)
{
    ...
    hbmOldMem = (HBITMAP)SelectObject(hdcMem, hbm);
    ...
    /* generate rtf stream */
    sz = GetEnhMetaFileBits(hEMF, 0, (LPBYTE)NULL);
    if (sz && (data = HeapAlloc(GetProcessHeap(), 0, sz)))
    {
        if (sz == GetEnhMetaFileBits(hEMF, sz, (LPBYTE)data))
        {
...
static BOOL HLPFILE_RtfAddBitmap(struct RtfData* rd, HLPFILE* file, const BYTE* beg, BYTE type, BYTE pack)
{
    ...
    bi = (BITMAPINFO*)HeapAlloc(GetProcessHeap(), 0, sizeof(*bi));
    if (!bi) return FALSE;
    ...
    if (type == 0x06)
    {
        ...
        bi = (BITMAPINFO*)HeapReAlloc(GetProcessHeap(), 0, bi, sizeof(*bi) + nc * sizeof(RGBQUAD));
        if (!bi) return FALSE;
        for (i = 0; i < nc; i++)
        {
    ...
    if (type == 0x06)
    {
        qsnprintf(tmp,sizeof(tmp), "\\dibitmap0\\picw%d\\pich%d",
                bi->bmiHeader.biWidth, bi->bmiHeader.biHeight);
    ...
    {
        qsnprintf(tmp,sizeof(tmp), "\\wbitmap0\\wbmbitspixel%d\\wbmplanes%d\\picw%d\\pich%d",
...
static BOOL     HLPFILE_RtfAddMetaFile(struct RtfData* rd, HLPFILE* file, const BYTE* beg, BYTE pack)
{
    ...
    qsnprintf(tmp,sizeof(tmp), "{\\pict\\wmetafile%d\\picw%d\\pich%d",
                                         ...
                                         unsigned index)
{
    ...
    qsnprintf(tmp,sizeof(tmp), "|bm%u", index);
                                             ...
                                             unsigned clrChange, unsigned bHotSpot, unsigned wnd)
{
    ...
    link = (HLPFILE_LINK*)HeapAlloc(GetProcessHeap(), 0, asz + len + 1);
    if (!link) return NULL;

	link->cookie     = (tagHlpFileLink::CK)cookie;
                                    ...
                                    BYTE *buf, BYTE* end, unsigned* parlen)
{
    ...
    *parlen = 0;
    ...
    text = text_base = (char*)HeapAlloc(GetProcessHeap(), 0, size);
    ...
    if (buf[0x14] == 0x23)
    {
        ...
        {
            int     pos;
            qsnprintf(tmp,sizeof(tmp), "\\trgaph%d\\trleft%d",
            ...
            for (nc = 0; nc < ncol; nc++)
            {
                ...
                qsnprintf(tmp,sizeof(tmp), "\\cellx%d",
        ...
        else
        {
            ...
            qsnprintf(tmp,sizeof(tmp), "\\trleft%d\\cellx%d ",
                    HLPFILE_HalfPointsToTwips(MulDiv(GET_SHORT(format, 0), table_width, 32767)),
    ...
    for (nc = 0; nc < ncol; /**/)
    {
        ...
        if (bits & 0x0002)
        {
            qsnprintf(tmp,sizeof(tmp), "\\sb%d", HLPFILE_HalfPointsToTwips(fetch_short(&format)));
            if (!HLPFILE_RtfAddControl(rd, tmp)) goto done;
        }
        if (bits & 0x0004)
        {
            qsnprintf(tmp,sizeof(tmp), "\\sa%d", HLPFILE_HalfPointsToTwips(fetch_short(&format)));
            if (!HLPFILE_RtfAddControl(rd, tmp)) goto done;
        }
        if (bits & 0x0008)
        {
            qsnprintf(tmp,sizeof(tmp), "\\sl%d", HLPFILE_HalfPointsToTwips(fetch_short(&format)));
            if (!HLPFILE_RtfAddControl(rd, tmp)) goto done;
        }
        if (bits & 0x0010)
        {
            qsnprintf(tmp,sizeof(tmp), "\\li%d", HLPFILE_HalfPointsToTwips(fetch_short(&format)));
            if (!HLPFILE_RtfAddControl(rd, tmp)) goto done;
        }
        if (bits & 0x0020)
        {
            qsnprintf(tmp,sizeof(tmp), "\\ri%d", HLPFILE_HalfPointsToTwips(fetch_short(&format)));
            if (!HLPFILE_RtfAddControl(rd, tmp)) goto done;
        }
        if (bits & 0x0040)
        {
            qsnprintf(tmp,sizeof(tmp), "\\fi%d", HLPFILE_HalfPointsToTwips(fetch_short(&format)));
            if (!HLPFILE_RtfAddControl(rd, tmp)) goto done;
        }
        if (bits & 0x0100)
        {
            ...
            if (w)
            {
                qsnprintf(tmp,sizeof(tmp), "\\brdrw%d", HLPFILE_HalfPointsToTwips(w));
                if (!HLPFILE_RtfAddControl(rd, tmp)) goto done;
            }
        }
        if (bits & 0x0200)
        {
            ...
            for (i = 0; i < ntab; i++)
            {
                ...
                /* FIXME: do kind */
                qsnprintf(tmp,sizeof(tmp), "%s\\tx%d",
        ...
        {
            WINE_TRACE("Got text: %s (%p/%p - %p/%p)\n", /*wine_dbgstr_a(text)*/ text, text, text_end, format, format_end);
            textsize = strlen(text);
            if (textsize)
            {
                if (rd->force_color)
                {
					if ((rd->current_link->cookie == tagHlpFileLink::hlp_link_popup) ?
	    ...
	    case 0x80:
                {
                    ...
                    qsnprintf(tmp,sizeof(tmp), "\\f%d\\cf%d\\fs%d%s%s%s%s",
                            font, font + 2, fs,
	    ...
	    case 0xA9:
            ...
            case 0xCC:
                WINE_TRACE("macro => %s\n", format + 3);
                HLPFILE_AllocLink(rd, tagHlpFileLink::hlp_link_macro, (const char*)format + 3,
            ...
            case 0xE1:
                ...
                HLPFILE_AllocLink(rd, (*format & 1) ? tagHlpFileLink::hlp_link_link : tagHlpFileLink::hlp_link_popup,
                           ...
                           unsigned font_scale, unsigned relative)
{
    ...
    rd->in_text = TRUE;
    rd->data = rd->ptr =(char*) HeapAlloc(GetProcessHeap(), 0, rd->allocated = 32768);
    rd->char_pos = 0;
...
//    case EE_CHARSET:            cpg = 1250; break;
    ...
    if (ck)
    {
        qsnprintf(tmp,sizeof(tmp), "{\\rtf1\\%s\\deff0", ck);
        if (!HLPFILE_RtfAddControl(rd, tmp)) return FALSE;
    }
    else
    {
        qsnprintf(tmp,sizeof(tmp), "{\\rtf1\\ansi\\ansicpg%d\\deff0", cpg);
        if (!HLPFILE_RtfAddControl(rd, tmp)) return FALSE;
    }
    ...
    for (index = 0; index < hlpfile->numFonts; index++)
    {
        ...
        qsnprintf(tmp,sizeof(tmp), "{\\f%d\\f%s\\fprq%d\\fcharset%d %s;}",
                index, family,
    ...
    for (index = 0; index < hlpfile->numFonts; index++)
    {
        ...
        qsnprintf(tmp,sizeof(tmp), "\\red%d\\green%d\\blue%d;",
...
static BOOL HLPFILE_ReadFont(HLPFILE* hlpfile)
{
    ...
    hlpfile->fonts = (HLPFILE_FONT*)HeapAlloc(GetProcessHeap(), 0, sizeof(HLPFILE_FONT) * dscr_num);
...
/* EPP         WINE_FIXME("[%d]: %*s\n", i / len, len, ref + i); */
    for (i = 0; i < dscr_num; i++)
    {
        ...
        else
        {
            ...
            qstrncpy((char*)hlpfile->fonts[i].LogFont.lfFaceName, "Helv",sizeof(hlpfile->fonts[i].LogFont.lfFaceName));
        }
...
static BOOL HLPFILE_ReadFileToBuffer(HLPFILE* hlpfile, HFILE hFile)
{
    ...
    hlpfile->file_buffer =(BYTE*) HeapAlloc(GetProcessHeap(), 0, hlpfile->file_buffer_size + 1);
...
static BOOL HLPFILE_SystemCommands(HLPFILE* hlpfile)
{
    ...
    if (hlpfile->version <= 16)
    {
        ...
        hlpfile->lpszTitle = (LPSTR)HeapAlloc(GetProcessHeap(), 0, strlen(str) + 1);
	...
	case 2:
            ...
            hlpfile->lpszCopyright = (LPSTR)HeapAlloc(GetProcessHeap(), 0, strlen(str) + 1);
	...
	case 4:
            macro = (HLPFILE_MACRO*)HeapAlloc(GetProcessHeap(), 0, sizeof(HLPFILE_MACRO) + lstrlen(str) + 1);
            if (!macro) break;
	    ...
	    if (hlpfile->windows) 
        	hlpfile->windows = (HLPFILE_WINDOWINFO*)HeapReAlloc(GetProcessHeap(), 0, hlpfile->windows, 
                                           sizeof(HLPFILE_WINDOWINFO) * ++hlpfile->numWindows);
	    else 
        	hlpfile->windows = (HLPFILE_WINDOWINFO*)HeapAlloc(GetProcessHeap(), 0, 
                                           sizeof(HLPFILE_WINDOWINFO) * ++hlpfile->numWindows);
            ...
            if (hlpfile->windows)
            {
                ...
                if (flags & 0x0001) qstrncpy(wi->type, &str[2],sizeof(wi->type));
                else wi->type[0] = '\0';
                if (flags & 0x0002) qstrncpy(wi->name, &str[12],sizeof(wi->name));
                else wi->name[0] = '\0';
                if (flags & 0x0004) qstrncpy(wi->caption, &str[21],sizeof(wi->caption));
    ...
    if (!hlpfile->lpszTitle)
        hlpfile->lpszTitle = (LPSTR)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, 1);
    return TRUE;
}
...
static BOOL HLPFILE_GetContext(HLPFILE *hlpfile)
{
    ...
    clen = cend - cbuf;
    hlpfile->Context = (BYTE*)HeapAlloc(GetProcessHeap(), 0, clen);
 ...
 *           HLPFILE_GetKeywords
 */
BOOL HLPFILE_GetKeywords(HLPFILE *hlpfile)
{
    BYTE                *cbuf, *cend;
    unsigned            clen;
    ...
    clen = cend - cbuf;
    hlpfile->kwbtree = (BYTE*)HeapAlloc(GetProcessHeap(), 0, clen);
    ...
    clen = cend - cbuf;
    hlpfile->kwdata = (BYTE*)HeapAlloc(GetProcessHeap(), 0, clen);
    if (!hlpfile->kwdata)
    {
...
static BOOL HLPFILE_GetMap(HLPFILE *hlpfile)
{
    ...
    hlpfile->Map = (HLPFILE_MAP*)HeapAlloc(GetProcessHeap(), 0, entries * sizeof(HLPFILE_MAP));
...
static BOOL HLPFILE_GetTOMap(HLPFILE *hlpfile)
{
    ...
    clen = cend - cbuf - 9;
    hlpfile->TOMap = (unsigned int*)HeapAlloc(GetProcessHeap(), 0, clen);
...
static BOOL HLPFILE_UncompressLZ77_Phrases(HLPFILE* hlpfile)
{
    ...
    hlpfile->phrases_offsets = (unsigned int*)HeapAlloc(GetProcessHeap(), 0, sizeof(unsigned) * (num + 1));
    hlpfile->phrases_buffer  = (char*)HeapAlloc(GetProcessHeap(), 0, dec_size);
...
static BOOL HLPFILE_Uncompress_Topic(HLPFILE* hlpfile)
{
    ...
    if (hlpfile->compressed)
    {
        ...
        hlpfile->topic_map = (BYTE**)HeapAlloc(GetProcessHeap(), 0,
...
static BOOL HLPFILE_AddPage(HLPFILE *hlpfile, const BYTE *buf, const BYTE *end, unsigned ref, unsigned offset)
{
    ...
    page = (HLPFILE_PAGE*)HeapAlloc(GetProcessHeap(), 0, sizeof(HLPFILE_PAGE) + titlesize + 1);
    ...
    while (ptr < page->lpszTitle + titlesize)
    {
        ...
        WINE_TRACE("macro: %s\n", ptr);
        macro = (HLPFILE_MACRO*)HeapAlloc(GetProcessHeap(), 0, sizeof(HLPFILE_MACRO) + len + 1);
...
HLPFILE *HLPFILE_ReadHlpFile(LPCSTR lpszPath)
{
    ...
    hlpfile = (HLPFILE*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
                        sizeof(HLPFILE) + lstrlen(lpszPath) + 1);
    if (!hlpfile) return 0;

    hlpfile->lpszPath           = (LPSTR)malloc(strlen(lpszPath)+1);//(char*)hlpfile + sizeof(HLPFILE);
    hlpfile->contents_start     = 0xFFFFFFFF;
    hlpfile->next               = first_hlpfile;
    hlpfile->wRefCount          = 1;

	qstrncpy(hlpfile->lpszPath, lpszPath,strlen(lpszPath)+1);
