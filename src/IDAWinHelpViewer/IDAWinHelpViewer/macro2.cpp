...
{
    char* dst;
    dst=(char*)HeapAlloc(GetProcessHeap(),0,strlen(str)+1);
    qstrncpy(dst, str,sizeof(dst));
    return dst;
}
...
void CALLBACK MACRO_CreateButton(LPCSTR id, LPCSTR name, LPCSTR macro)
{
    ...
    button = (WINHELP_BUTTON*)HeapAlloc(GetProcessHeap(), 0, size);
    if (!button) return;
...
void CALLBACK MACRO_Back(void)
{
	...
	if (win && win->back.index >= 1)
	{
		WINHELP_WNDPAGE wpage = win->back.set[(--win->back.index) - 1];
		win->page = wpage.page;
...
static void CALLBACK MACRO_JumpID(LPCSTR lpszPathWindow, LPCSTR topic_id)
{
    ...
    if ((ptr = (LPSTR)strchr(lpszPathWindow, '>')) != NULL)
    {
        LPSTR   tmp;
        size_t  sz = ptr - lpszPathWindow;

        tmp = (LPSTR) HeapAlloc(GetProcessHeap(), 0, sz + 1);
        if (tmp)
        {
...
static void CALLBACK MACRO_RegisterRoutine(LPCSTR dll_name, LPCSTR proc, LPCSTR args)
{
    ...
    if (!dll)
    {
        ...
        else if ((dll = (WINHELP_DLL*)HeapAlloc(GetProcessHeap(), 0, sizeof(*dll))))
        {
            dll->hLib = hLib;
            dll->name = StrDup(dll_name); /* FIXME: never freed */
            dll->next = Globals.dlls;
            Globals.dlls = dll;
            dll->handler = (WINHELP_LDLLHandler)GetProcAddress((HMODULE)dll->hLib, "LDLLHandler");
            dll->tclass = dll->handler ? (dll->handler)(DW_WHATMSG, 0, 0) : DC_NOMSG;
            WINE_TRACE("Got class %x for DLL %s\n", dll->tclass, dll_name);
            if (dll->tclass & DC_INITTERM) dll->handler(DW_INIT, 0, 0);
            if (dll->tclass & DC_CALLBACKS) dll->handler(DW_CALLBACKS, (LONG_PTR)&Callbacks, 0);
        }
        else WINE_WARN("OOM\n");
    }
    if (dll && !(fn = GetProcAddress((HMODULE)dll->hLib, proc)))
    {
        /* FIXME: internationalisation for error messages */
    ...
    if (!MACRO_Loaded) MACRO_Loaded = (MacroDesc*)HeapAlloc(GetProcessHeap(), 0, size);
    else MACRO_Loaded = (MacroDesc*)HeapReAlloc(GetProcessHeap(), 0, MACRO_Loaded, size);
...
static void CALLBACK MACRO_SetHelpOnFile(LPCSTR str)
{
    ...
    page->file->help_on_file = (LPSTR)HeapAlloc(GetProcessHeap(), 0, strlen(str) + 1);
    if (page->file->help_on_file)
        qstrncpy(page->file->help_on_file, str,sizeof(page->file->help_on_file));
}
...
static int MACRO_DoLookUp(struct MacroDesc* start, const char* name, struct lexret* lr, unsigned len)
{
    ...
    {
		if (strnicmp(md->name, name,sizeof(md->name)) == 0 || (md->alias != NULL && strnicmp(md->alias, name,sizeof(md->name)) == 0))
        {
            lr->proto = md->arguments;
            lr->function = (FARPROC)md->fn;
